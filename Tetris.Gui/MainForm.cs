...
namespace Tetris.Gui
{
    public partial class MainFrom : Form
    {
        ...
        {
            var result = MessageBox.Show("Выберите сложность игры:", "Выбор сложности", MessageBoxButtons.YesNoCancel);
        ...
        private void ClearHoldTetromino()
        {
            if (holdTetrominoPicBox.InvokeRequired)
            {
                holdTetrominoPicBox.Invoke(new Action(() =>
                {
        ...
        {
            tetrisGame.Pause();
            if (MessageBox.Show(this, "Are u sure u want to change Difficulty? the progress will be lost 💔", "Difficulty♡", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                if (tetrisGame.Difficulty == "Hard")
                {
                    tetrisGame.Difficulty = "Easy";
                }
                else
                {
                    tetrisGame.Difficulty = "Hard";
                }
            }
            tetrisGame.Restart();
        }
        ...
        private void DrawBoarder()
        {
            ...
            {
                graphic.DrawRectangle(new Pen(Color.Crimson, deckBorderWidth), new Rectangle(0, 0, gameDeckPicBox.Width, gameDeckPicBox.Height));
        ...
        private Color GetColor(BlockStatus status, Color hiddenColor)
        {
            switch (status)
            {
                ...
                /// <summary>
                /// Extra
                case BlockStatus.Salmon:
                    return Color.Salmon;

                case BlockStatus.Tan:
                    return Color.Tan;

                case BlockStatus.DarkKhaki:
                    return Color.DarkKhaki;

                case BlockStatus.DarkSeaGreen:
                    return Color.DarkSeaGreen;

                case BlockStatus.PaleGreen:
                    return Color.PaleGreen;

                case BlockStatus.MediumAquamarine:
                    return Color.MediumAquamarine;

                case BlockStatus.LightSkyBlue:
                    return Color.LightSkyBlue;

                case BlockStatus.SteelBlue:
                    return Color.SteelBlue;

                case BlockStatus.MediumPurple:
                    return Color.MediumPurple;

                case BlockStatus.MediumOrchid:
                    return Color.MediumOrchid;

                case BlockStatus.MediumVioletRed:
                    return Color.MediumVioletRed;

                case BlockStatus.PaleVioletRed:
        ...
        private void DecorateFormControls()
        {
            ...
            {
                if (control is Label label && label.Text == "Tetris♡")
                {
            ...
            scoreGrp.BackColor = ColorTranslator.FromHtml("#FFFFF0F5");

            shortcutsGrp.Left = scoreGrp.Left;
            shortcutsGrp.Top = scoreGrp.Top + scoreGrp.Height + verticalSpaceBetweenUiElements;
            shortcutsGrp.BackColor = ColorTranslator.FromHtml("#FFFFF0F5");
            ...
            Label tetrisLabel = new Label();
            tetrisLabel.Text = "Tetris♡";
            tetrisLabel.Font = new Font("Arial", 16, FontStyle.Bold);
            tetrisLabel.AutoSize = true;

            tetrisLabel.Left = (Width - tetrisLabel.Width) / 2;
            tetrisLabel.Top = gameDeckPicBox.Top - tetrisLabel.Height - 10;

            // Добавление надписи "Tetris" на форму
        ...
        private void InitializeTetrisGame()
        {
            ...
            tetrisGame = new Game.Tetris(deckWidth, deckHeight, gameDifficulty);
            ...
            DrawBoarder();
            this.BackColor = ColorTranslator.FromHtml("#FFFFE4E1");
            gameDeckPicBox.Left = (Width - gameDeckPicBox.Width - scoreGrp.Width - nextTetrominoPicBox.Width - horizontalSpaceBetweenUiElements * 2 - holdTetrominoPicBox.Width) / 2;
            gameDeckPicBox.Top = (Height - gameDeckPicBox.Height) / 2;

            tetrisGame.Pause();
            if (MessageBox.Show(this, "Do u want this game to be easy?♡", "Difficulty♡", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                tetrisGame.Difficulty = "Hard";
            }
        ...
        private void UpdateScores(ScoreEventArgs scoreEventArgs)
        {
            ...
            rows = scoreEventArgs.Lines;
            level = scoreEventArgs.Level.ToString();
            score = scoreEventArgs.Score;
        ...
        private void GameOver()
        {
            ...
            if (MessageBox.Show(this, "Do you want to save results in a file?♡", "Game Over", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                SaveGameDataToFile();
            }
        }
        ...
        private void SaveGameDataToFile()
        {
            ...
            {
                writer.WriteLine("♡Lines: " + rows.ToString());
                writer.WriteLine("♡Score: " + score.ToString());
                writer.WriteLine("♡Level: " + level.ToString());
            }

            MessageBox.Show(this, "Game results saved to " + fileName);
        }
